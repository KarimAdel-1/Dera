...
interface IDeraSupplyToken is IERC20, IScaledBalanceToken, IInitializableDeraSupplyToken {
  ...
  function burn(address from, address receiverOfUnderlying, uint256 amount, uint256 scaledAmount, uint256 index) external returns (bool);
  function mintToTreasury(uint256 scaledAmount, uint256 index) external;
  function transferOnLiquidation(address from, address to, uint256 amount, uint256 scaledAmount, uint256 index) external;
