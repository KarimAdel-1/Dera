...
async function main() {
  ...
  try {
    ...
    // Update frontend .env.local (preserve existing credentials!)
    const envPath = path.join(__dirname, "../../frontend/.env.local");
    let frontendEnvContent = fs.existsSync(envPath)
    ...
    // Update deployment-related variables only (preserves Supabase, WalletConnect, etc.)
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_HEDERA_NETWORK', 'testnet');
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_POOL_ADDRESS', addresses.POOL);
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_ORACLE_ADDRESS', addresses.ORACLE);
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_ANALYTICS_ADDRESS', addresses.ANALYTICS);
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_MULTI_ASSET_STAKING_ADDRESS', addresses.MULTI_ASSET_STAKING);
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_NODE_STAKING_ADDRESS', addresses.MULTI_ASSET_STAKING);
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_POOL_CONFIGURATOR', addresses.POOL_CONFIGURATOR);
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_RATE_STRATEGY', addresses.RATE_STRATEGY);
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_ACL_MANAGER', addresses.ACL_MANAGER);
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_PROVIDER', addresses.POOL_ADDRESSES_PROVIDER);
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_NATIVE_HBAR_ENABLED', 'true');
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_HBAR_DECIMALS', '8');
    frontendEnvContent = updateOrAppend(frontendEnvContent, 'NEXT_PUBLIC_HBAR_ACTIVE', addresses.NATIVE_HBAR_ACTIVE || false);

    fs.writeFileSync(envPath, frontendEnvContent);
